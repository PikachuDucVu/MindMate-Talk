...
Vị trí:        Header của màn hình Chat (Voice & Text)
Icon:          🆘 hoặc icon "Phao cứu sinh" (lifebuoy)
Màu sắc:       Teal/Blue nhạt (KHÔNG dùng màu đỏ để tránh gây hoảng)
Trạng thái:    Luôn hiển thị, không bị ẩn khi scroll
Accessibility: Có aria-label "Hỗ trợ khẩn cấp"

MỤC ĐÍCH:
- Cho phép user gọi hotline ngay lập tức
- Không cần đợi AI phát hiện từ khóa
- User chủ động tìm kiếm hỗ trợ khi cần
```

### 6.2 SOS Button Tap Flow
...
│              Bạn cần hỗ trợ ngay?                                   │
│                                                                      │
│  Nếu bạn đang trong tình trạng khẩn cấp, hãy liên hệ:              │
│                                                                      │
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │  📞  Đường dây Sức khỏe Tâm thần                              │  │
│  │      1800-599-920                                             │  │
...
│  ─────────────────────────────────────────────────────────────────  │
│                                                                      │
│  Bạn cũng có thể tiếp tục nói chuyện với MindMate.                 │
...
### 6.3 SOS Button Design Guidelines
...
DO (Nên làm):
├── Dùng màu xanh dương/teal nhẹ nhàng
├── Icon rõ ràng, dễ nhận biết
├── Luôn hiển thị trong tầm nhìn
├── Bottom sheet mở mượt mà
├── Cho phép đóng dễ dàng
└── Không yêu cầu xác nhận phức tạp

DON'T (Không nên):
├── Dùng màu đỏ chói (gây hoảng loạn)
├── Làm nút quá to (áp lực user)
├── Popup chặn toàn màn hình
├── Yêu cầu nhiều bước để gọi
├── Tự động gọi khi tap (cần xác nhận)
└── Ghi log việc tap SOS (privacy)
```
